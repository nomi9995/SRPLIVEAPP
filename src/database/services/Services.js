  ...
  insertAndUpdateUserListonlogin: (tableName, data, onlineUser, callback) => {
    ...
    data.forEach(element => {
      ...
      let p = new Promise((resolve, reject) => {
        ChatUsersQuieries.checkUserExsist({ userId, onlineUser, isroom }, async res => {
...
export const MessagesQuieries = {
  ...
  getSearchBasedMsgs: async (params, callback) => {
    SQLiteDB().transaction(tx => {
      tx.executeSql(
        ...
        error => {
          console.log("Error on getting search message list data: ", error.message)
        }
      );
    });
  ...
  getMessageOffset: (params, callback) => {
    SQLiteDB().transaction(tx => {
      tx.executeSql(
        ...
        error => {
          console.log("Error on getting offset message list data: ", error.message)
        }
      );
    });
  },
  ...
  updateMessageAction: (params, callBack) => {
    SQLiteDB().transaction(tx => {
      tx.executeSql(
        ...
        status=?
        where id=${params.deleteMeassgeId} AND onlineUser=${params.onlineUserId
  ...
  updateMessageActionAsStar: (params, callBack) => {
    SQLiteDB().transaction(tx => {
      tx.executeSql(
        `UPDATE messages_list_table set
        my_star=?
        where id=${params.messageId} AND onlineUser=${params.onlineUserId
        } AND chatUser=${JSON.parse(params.chatUserId)}`,
        [params.star],
        (tx, results) => {
          if (results.rowsAffected !== 0) {
  ...
  delete: (tableName, data) => {
    SQLiteDB().transaction(tx => {
      tx.executeSql(
        'Drop table messages_list_table',
        [],
        (tx, results) => {
          if (results.rowsAffected > 0) {
            // Alert.alert(
