...
class StarredList extends Component {
  ...
  getAllMsgsFromDb = () => {
    ...
    let mystar = '1'
    MessagesQuieries.selectDbSingleListmessage({ onlineUserId, chatUserId , mystar }, res2 => {
              ...
              <>
                <View style={styles.starredInnerView}>
                  <View style={styles.senderNameAndReceiverView}>
                    <Image source={require('../../assets/deafultimage.png')} style={styles.senderReciverImage} />
                    <Text style={styles.senderRevicerText}>{messages?.first_name+' ' +messages?.last_name}</Text>
                    <View style={styles.Icon}>
                      <FontAwesome name={'play'} size={5} color={'white'} />
                    </View>
                    {this.props?.route?.params?.selectedUser?.chat_name === undefined ? 
                    <Text style={styles.youText}>{messages?.to_user_firstname+' ' +messages?.to_user_lastname}</Text>
                  : messages?.first_name == this.props.user.user.first_name ? 
                  <Text style={styles.youText}>{this.props?.route?.params?.selectedUser?.chat_name}</Text>
                  :
                  <Text style={styles.youText}>{this.props?.user?.user?.first_name + ' ' + this.props?.user?.user?.last_name}</Text>
                  }
                  </View>
                ...
                <View style={styles.starredMessage}>
                  ...
                  <View style={styles.starAndTime}>
                    <FontAwesome5 name={'star'} size={10} color={'black'} style={styles.starIcon} />
                    <Text style={styles.starredMessagetime}>{moment(messages?.starred_at).format("LT")}</Text>
                  </View>
                </View>
              </>
            )
