...
//Redux
import { setOnLongPress, setReplyState, setMessageEdit, setMessageText } from '../store/actions';
      ...
      <View style={styles.container}>
        <View style={styles.mainWrapper}>
          <View style={styles.leftWrapper}>
            <Text style={styles.nameText}>{longPress[0]?.user._id === 2 ? 'You' : longPress[0]?.first_name}</Text>

            {longPress[0]?.type === 1 && (
              <Text style={styles.plainMsgText} numberOfLines={1}>{longPress[0]?.message}</Text>
            )}
          ...
          <TouchableOpacity style={styles.closeBtnWraper} onPress={() => this.onPressCross()}>
